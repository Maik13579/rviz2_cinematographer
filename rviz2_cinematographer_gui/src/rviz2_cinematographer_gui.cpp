...
void Rviz2CinematographerGUI::initPlugin(qt_gui_cpp::PluginContext& context)
{
  ...
  node_ = rclcpp::Node::make_shared("rviz2_cinematographer_gui_node");
  camera_trajectory_pub_ = node_->create_publisher<rviz2_cinematographer_msgs::msg::CameraTrajectory>(
    "/rviz2/camera_trajectory",
    rclcpp::QoS(rclcpp::KeepLast(10))
  );
  view_poses_array_pub_ = node_->create_publisher<nav_msgs::msg::Path>(
      "/transformed_path",
      rclcpp::QoS(rclcpp::KeepLast(10))
  );
  record_params_pub_ = node_->create_publisher<rviz2_cinematographer_msgs::msg::Record>(
      "/rviz2/record",
      rclcpp::QoS(rclcpp::KeepLast(10))
  );
  // Create the QWidget
  ...
  menu_handler_.insert("Add marker before", [this](const visualization_msgs::msg::InteractiveMarkerFeedback::ConstSharedPtr& feedback) {
      this->addMarkerBeforeClicked(feedback);
  });

  menu_handler_.insert("Add marker here", [this](const visualization_msgs::msg::InteractiveMarkerFeedback::ConstSharedPtr& feedback) {
      this->addMarkerHereClicked(feedback);
  });

  menu_handler_.insert("Add marker after", [this](const visualization_msgs::msg::InteractiveMarkerFeedback::ConstSharedPtr& feedback) {
      this->addMarkerBehindClicked(feedback);
  });

  menu_handler_.insert("Remove marker", [this](const visualization_msgs::msg::InteractiveMarkerFeedback::ConstSharedPtr& feedback) {
  ...
  if(node_->has_parameter(poses_param_name))
  {
    loadParams(node_, poses_param_name);
  }
  else
  {
  ...
  record_finished_sub_ = node_->create_subscription<rviz2_cinematographer_msgs::msg::Finished>(
    ...
    10,
    [this](const rviz2_cinematographer_msgs::msg::Finished::SharedPtr msg) {
  ...
  node_->get_parameter("start_recorder", start_recorder);
    ...
    //video_recorder_thread_ = std::make_shared<std::thread>(&Rviz2CinematographerGUI::videoRecorderThread, this);  else
  {
    ...
    RCLCPP_WARN(node_->get_logger(), "Video recorder was not started.");
  }
}
...
void Rviz2CinematographerGUI::camPoseCallback(const geometry_msgs::msg::Pose::ConstSharedPtr& cam_pose)
{
  cam_pose_ = geometry_msgs::msg::Pose(*cam_pose);
  server_->applyChanges();
}

void Rviz2CinematographerGUI::setUpTableHeader(QTableWidget* marker_table_widget)
{
...
void Rviz2CinematographerGUI::deleteTableContents(QTableWidget* marker_table_widget)
{
...
void Rviz2CinematographerGUI::recordFinishedCallback(const rviz2_cinematographer_msgs::msg::Finished::SharedPtr record_finished)
{
...
void Rviz2CinematographerGUI::loadTrajectoryFromFile()
{
    ...
    {
        RCLCPP_ERROR(node_->get_logger(), "No file specified.");
        return;
    }
  ...
  if(extension == ".yaml")
  {
  ...
  else if(extension == ".txt")
    {
      ...
      while(std::getline(infile, line))
      {
          ...
          {
              RCLCPP_ERROR(node_->get_logger(), "Line: %s contains the wrong number of parameters. Format is: timestamp tx ty tz qx qy qz qw. Number of parameters are %zu", line.c_str(), pose_strings.size());
              continue;
          }
  ...
  {
      RCLCPP_ERROR(node_->get_logger(), "Specified file is neither .yaml nor .txt file.\n File name is: %s", file_path.string().c_str());    return;
  }
...
tf2::Vector3 Rviz2CinematographerGUI::rotateVector(const tf2::Vector3& vector, const geometry_msgs::msg::Quaternion& quat_msg)
{
    tf2::Quaternion quat;
    tf2::fromMsg(quat_msg, quat);
    tf2::Vector3 rotated_vector = tf2::quatRotate(quat, vector);
...
visualization_msgs::msg::InteractiveMarker Rviz2CinematographerGUI::makeMarker(double x,
                                                                         double y,
                                                                         double z)
{
...
void Rviz2CinematographerGUI::appendMarkerToTrajectory(const MarkerIterator& goal_marker_iter, 
    std::shared_ptr<rviz2_cinematographer_msgs::msg::CameraTrajectory>& cam_trajectory, 
...
void Rviz2CinematographerGUI::processFeedback(const visualization_msgs::msg::InteractiveMarkerFeedback::ConstSharedPtr& feedback)
{
  // update markers
  visualization_msgs::msg::InteractiveMarker marker;
...
void Rviz2CinematographerGUI::addMarkerBeforeClicked(const visualization_msgs::msg::InteractiveMarkerFeedback::ConstSharedPtr& feedback)
{
...
void Rviz2CinematographerGUI::addMarkerBefore(const std::string& current_marker_name)
{
  ...
  if(clicked_element != markers_.end())
  {
    ...
    if(pose_before_initialized && clicked_pose_initialized)
    {
      ...
      tf2::fromMsg(pose_before.orientation, start_orientation);
      tf2::fromMsg(clicked_pose.orientation, end_orientation);
      intermediate_orientation = start_orientation.slerp(end_orientation, 0.5);
      new_marker.pose.orientation = tf2::toMsg(intermediate_orientation);
    }
    else
    {
...
void Rviz2CinematographerGUI::addMarkerAtClicked(const visualization_msgs::msg::InteractiveMarkerFeedback::ConstSharedPtr& feedback)
{
  setCurrentFromTo(getMarkerByName(current_marker_name_), getMarkerByName(feedback->marker_name));
...
void Rviz2CinematographerGUI::addMarkerBehind(const std::string& current_marker_name)
{
  ...
  if(clicked_element != markers_.end())
  {
    ...
    if(clicked_pose_initialized && pose_behind_initialized)
    {
      // Compute the slerp-ed rotation
      tf2::Quaternion start_orientation, end_orientation, intermediate_orientation;
      tf2::fromMsg(clicked_pose.orientation, start_orientation);
      tf2::fromMsg(pose_behind.orientation, end_orientation);
      intermediate_orientation = start_orientation.slerp(end_orientation, 0.5);
      new_marker.pose.orientation = tf2::toMsg(intermediate_orientation);
    }
    else
...
void Rviz2CinematographerGUI::addMarkerBehindClicked(const visualization_msgs::msg::InteractiveMarkerFeedback::ConstSharedPtr& feedback)
{
  setCurrentFromTo(getMarkerByName(current_marker_name_), getMarkerByName(feedback->marker_name));
...
void Rviz2CinematographerGUI::removeClickedMarker(const visualization_msgs::msg::InteractiveMarkerFeedback::ConstSharedPtr& feedback)
{
  setCurrentFromTo(getMarkerByName(current_marker_name_), getMarkerByName(feedback->marker_name));
...
void Rviz2CinematographerGUI::removeMarker(const std::string& marker_name)
{
  if(markers_.size() < 3)
  {
    RCLCPP_ERROR(node_->get_logger(), "Cannot remove last two markers.");
    return;
  }
...
void Rviz2CinematographerGUI::removeCurrentMarker(const std_msgs::msg::Empty::SharedPtr& empty)
{
  clickButton(ui_.delete_push_button);
}
...
void Rviz2CinematographerGUI::updateServer(MarkerList& markers)
{
  ...
  for(auto& marker : markers)
  {
    ...
    count++;
    server_->insert(marker.marker, [this](const visualization_msgs::msg::InteractiveMarkerFeedback::ConstSharedPtr& feedback) {
...
void Rviz2CinematographerGUI::loadParams(const rclcpp::Node::SharedPtr node, const std::string& param_name)
{
    ...
    {
        RCLCPP_ERROR(node->get_logger(), "Parameter '%s' not found.", param_name.c_str());
        return;
    }

    // Retrieve the parameter as a string
    std::string yaml_str;
    if (!node->get_parameter(param_name, yaml_str))
    {
        RCLCPP_ERROR(node->get_logger(), "Failed to get parameter '%s' as string.", param_name.c_str());
        return;
    }
    ...
    {
        RCLCPP_ERROR(node->get_logger(), "Failed to parse YAML for parameter '%s': %s", param_name.c_str(), e.what());
        return;
    }

    if (!config.IsSequence())
    {
        RCLCPP_ERROR(node->get_logger(), "Parameter '%s' is not a sequence.", param_name.c_str());
        return;
    }

    // Iterate over the parameter array to construct markers
    for (size_t i = 0; i < config.size(); ++i)
    {
        const YAML::Node& v = config[i];
        if (!v.IsMap())
        {
            RCLCPP_WARN(node->get_logger(), "Pose %zu is not a map.", i);
            continue;
        }

        // Initialize the marker and set orientation and position
        visualization_msgs::msg::InteractiveMarker wp_marker = makeMarker();

        // Orientation
        if (v["orientation"])
        {
            wp_marker.pose.orientation.w = v["orientation"]["w"].as<double>();
            wp_marker.pose.orientation.x = v["orientation"]["x"].as<double>();
            wp_marker.pose.orientation.y = v["orientation"]["y"].as<double>();
            wp_marker.pose.orientation.z = v["orientation"]["z"].as<double>();
        }
        else
        {
            RCLCPP_WARN(node->get_logger(), "Pose %zu missing 'orientation' field.", i);
        }

        // Position
        if (v["position"])
        {
            wp_marker.pose.position.x = v["position"]["x"].as<double>();
            wp_marker.pose.position.y = v["position"]["y"].as<double>();
            wp_marker.pose.position.z = v["position"]["z"].as<double>();
        }
        else
        {
            RCLCPP_WARN(node->get_logger(), "Pose %zu missing 'position' field.", i);
        }

        // Set default color
        wp_marker.controls[0].markers[0].color.r = 1.f;
        wp_marker.controls[0].markers[0].color.g = 0.0f; // Optional: set other color channels if needed
        wp_marker.controls[0].markers[0].color.b = 0.0f;

        // Name and Description
        wp_marker.name = std::to_string(i + 1);
        wp_marker.description = std::to_string(i + 1);

        // Transition and Wait Durations
        double transition_duration = 2.5; // Default value
        double wait_duration = 2.0;       // Default value

        if (v["transition_duration"])
        {
            transition_duration = v["transition_duration"].as<double>();
        }
        else
        {
            RCLCPP_WARN(node->get_logger(), "Pose %zu missing 'transition_duration' field. Using default: %.2f", i, transition_duration);
        }

        if (v["wait_duration"])
        {
            wait_duration = v["wait_duration"].as<double>();
        }
        else
        {
            RCLCPP_WARN(node->get_logger(), "Pose %zu missing 'wait_duration' field. Using default: %.2f", i, wait_duration);
        }

        // Add the marker to the list
        markers_.emplace_back(TimedMarker(std::move(wp_marker), transition_duration, wait_duration));
    }
}
...
void Rviz2CinematographerGUI::setValueQuietly(QDoubleSpinBox* spin_box, double value)
{
                                                   ...
                                                   bool duplicate_ends)
{
  ...
  for(double i = 0.f; i <= max_t;)
  {
    ...
    tf2::Quaternion start_orientation, end_orientation, intermediate_orientation;
    tf2::fromMsg(current_marker->marker.pose.orientation, start_orientation);
    tf2::fromMsg(next_marker->marker.pose.orientation, end_orientation);
    ...
    pose.orientation = tf2::toMsg(intermediate_orientation);
...
void Rviz2CinematographerGUI::markersToSplinedCamTrajectory(
...
void Rviz2CinematographerGUI::splineToCamTrajectory(
  const UniformCRSpline<Vector3>& eye_spline,
  ...
  std::shared_ptr<rviz2_cinematographer_msgs::msg::CameraTrajectory> trajectory)
{
  ...
  for(double t = 0.0; t <= max_t;)
  {
    ...
    current_transition_id = static_cast<int>(std::floor(t + 0.00001)); // magic number needed due to arithmetic imprecision with doubles
